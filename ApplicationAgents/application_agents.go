...
// 406 Links
func ApplicationAgents() {
	data := getApplicationUserAgents()
	utils.CreateJSONUserAgents("ApplicationAgents/ApplicationUserAgents.json", data)
}

func getApplicationLinks() []string {
	var links []string = make([]string, 0)
	var url string = utils.ReadEnv("APPSAGENT")
...
func getApplicationUserAgents() []string {
	/*
	...
	*/
	links := utils.ReadLinks("ApplicationAgents/links.json")
	...
	c := colly.NewCollector(
		...
		colly.CacheDir("./ApplicationAgents/user-agent-cache"),
	)
	limitErr := c.Limit(&colly.LimitRule{
		DomainGlob:  "*",
		Delay:       2 * time.Second, // Add a 2-second delay between requests
