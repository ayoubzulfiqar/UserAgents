...
func DeviceAgent() {
	data := getDeviceUserAgents()
	utils.CreateJSONUserAgents("DeviceAgents/DeviceUserAgents.json", data)
}

func getDeviceLinks() []string {
	var links []string = make([]string, 0)
	var url string = utils.ReadEnv("DEVICESAGENT")
...
func getDeviceUserAgents() []string {
	/*
	...
	*/
	links := utils.ReadLinks("DeviceAgents/links.json")
	...
	c := colly.NewCollector(
		...
		colly.CacheDir("./DeviceAgents/user-agent-cache"),
	)
	limitErr := c.Limit(&colly.LimitRule{
		DomainGlob:  "*",
		Delay:       2 * time.Second, // Add a 2-second delay between requests
