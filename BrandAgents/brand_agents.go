...
// Total 885 Brand
func BrandAgents() {
	data := getBrandUserAgents()

	utils.CreateJSONUserAgents("BrandAgents/BrandUserAgents.json", data)
}

func getBrandLinks() []string {
	var links []string = make([]string, 0)
	var url string = utils.ReadEnv("BRANDAGENT")
	fmt.Println(url)
...
func getBrandUserAgents() []string {
	/*
	...
	*/
	links := utils.ReadLinks("BrandAgents/links.json")
	...
	c := colly.NewCollector(
		...
		colly.CacheDir("./BrandAgents/user-agent-cache"),
	)
	// limitErr := c.Limit(&colly.LimitRule{
	// 	DomainGlob:  "*",
	// 	Delay:       2 * time.Second, // Add a 2-second delay between requests
