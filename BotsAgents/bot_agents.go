...
func BotsAgents() {
	data := getBotUserAgents()
	utils.CreateJSONUserAgents("BotsAgents/BotUserAgents.json", data)
}

func getBotsLinks() []string {
	var links []string = make([]string, 0)
	var url string = utils.ReadEnv("BOTSAGENT")
...
func getBotUserAgents() []string {
	...
	*/
	links := utils.ReadLinks("BotsAgents/links.json")
	...
	c := colly.NewCollector(
		...
		colly.CacheDir("./BotsAgents/user-agent-cache"),
	)
	limitErr := c.Limit(&colly.LimitRule{
		DomainGlob:  "*",
		Delay:       2 * time.Second, // Add a 2-second delay between requests
